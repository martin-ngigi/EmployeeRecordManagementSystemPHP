...
<table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
  ...
  <?php //start of php 1
    ...
    // $query2=mysqli_query($con,"select * from employeeleave where EmpID=$empid" );
    // $row2=mysqli_fetch_array($query2);

    $result3=mysqli_query($con, "select * from employeeleave where EmpID=$empid AND ID=(SELECT max(id) FROM employeeleave)" );
    $row3= mysqli_fetch_assoc($result3);
    $status3 = "{$row3['LeaveStatus']}";

    //if there is a pending leave, dont show 'apply leave table'
    if($status3=='Pending') //start of if employeeleave
    { ?>
      <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
          <th>Leave Status</th>
          <td style="color:blue;" ><?php echo "Your Current leave status is still Pending Please wait till its processed." ?></td>
        </tr>
      </table>
    <?php } 
    //if status has been approved, show approval message
    elseif($status3=='Approved') //start of if employeeleave
    { ?>
      <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
          <th>Leave Status</th>
    ...
    <?php } 
    //if status has been Disapproved, show Disapproval message
    elseif($status3=='Disapproved') //start of if employeeleave
    { ?>
      <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
          <th>Leave Status</th>
...
<?php 
    } //end of if there is no pending leave, show 'apply leave table'
?>



<?php } //end of if employeedetail ?>
